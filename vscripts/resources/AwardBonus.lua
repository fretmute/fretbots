    ...
    if Settings.deathBonus.enabled[award] then
        ...
        for _, award in ipairs(Settings.deathBonus.order) do
            ...
            if awards > Settings.deathBonus.maxAwards then
                if isDebug then
                    print(bot.stats.name .. ": Max awards of " .. Settings.deathBonus.maxAwards .. " reached.")
                end
                break
            end
            -- make the award
...
function AwardBonus:GetValue(bot, award)
    ...
    -- set isLoud
    isLoud =
        (Settings.deathBonus.isClampLoud[award] and clamped == Settings.deathBonus.clamp[award][2]) or
...
function AwardBonus:ShouldAward(bot, award)
    ...
    if bot.stats.chance[award] >= 1 then
        if isDebug then
            print(bot.stats.name .. ": Chance for " .. award .. " was 1 or greater.")
        end
        return true
    end
    -- check timeGate
    ...
    if Settings.deathBonus.deathStreakThreshold >= 0 then
        if bot.stats.deathStreak >= Settings.deathBonus.deathStreakThreshold then
            if isDebug then
                print(
                    bot.stats.name ..
                        ": automatic " .. award .. " bonus due to death streak of " .. bot.stats.deathStreak .. "."
                )
            end
            return true
        end
