...
function DynamicDifficulty:Adjust(victim)
    ...
    if not Flags.isDynamicDifficultyFinalized then
        return
    end
    -- ensure we're enabled
    if not Settings.dynamicDifficulty.enabled then
        return
    end
    -- do not do anything for humans
    if not victim.stats.isBot then
        return
    end
    ...
    if bonus >= 0 then
        ...
        if Settings.dynamicDifficulty[knob].announce and bonus > 0 then
            ...
            local msg =
                msg ..
                "Adjusting Bot " ..
                    string.upper(knob) .. " Offset: " .. bonus .. " (" .. (increments + 1) .. " deficit increments)"
...
function DynamicDifficulty:AdjustDeathBonus(knob, bot)
    ...
    if bonus >= 0 then
        ...
        if Settings.dynamicDifficulty[knob].announce and bonus > 0 then
            ...
            local msg =
                msg ..
                "Adjusting Bot Death Bonus: " ..
                    knob .. " offset: " .. bonus .. " (" .. (increments + 1) .. " deficit increments)"
            Utilities:Print(msg, MSG_WARNING)
        end
    else
        ...
        if bonus > 0 then
            ...
            if Settings.dynamicDifficulty[knob].announce then
                ...
                local msg =
                    msg ..
                    "Adjusting Bot Death Bonus: " ..
                        knob .. " chance: " .. bonus .. " (" .. (increments + 1) .. " deficit increments)"
                Utilities:Print(msg, MSG_WARNING)
            end
        else
